...
- **N5 — Settings hub** (§4.5) + OIDC login button (§4.8 auth restyle rides along). **N5 — DONE (2026-07-13):** `/settings` hub (Placeholder → real page) with URL-driven left subnav + four sections — **Instance** (seal-status + typed-confirm Seal + fetch-blob backup download; version omitted, no `GET /v1/sys/version` yet), **OIDC provider** (view/edit/delete `/v1/sys/oidc`; client_secret write-only + required-on-save per the full-replace backend contract), **CI federation** (config + trust-bindings CRUD, match-claims editor, kit `Sheet` create), **Appearance** (theme default). OIDC/federation sections self-gate 403 (requires-admin hint) / 404 (not-configured empty state). Login page gains an SSO anchor behind `GET /v1/auth/oidc/status`. Frontend-only (all endpoints pre-existed). Closes gaps.md 1.1/1.3/1.4/1.6. Rider: fixed `EmptyState` `text-muted`→`text-ink-mute` N1 straggler (broader `text-muted` sweep across ~12 untouched files deferred to N7).
- **N6 — Ops create flows** (§4.6) + ErrorBoundary/404 (§4.7). **N6 — DONE (2026-07-13):** ops create Sheets for all three engines — rotation policy (postgres/webhook), sync target (github/**k8s**), dynamic role (SQL templates + `{{name}}`/`{{password}}`/`{{expiration}}` hint chips) — each with **write-only credential inputs** (DSN/PAT/kubeconfig creds/HMAC keys as `type=password`, ephemeral, reset on open, never rendered from a fetched value; masked create responses); shared `ConfigPicker` over the 403-tolerant `useProjectConfigMap`; removed the "use the CLI" empty-state copy. App-level `ErrorBoundary` (Nocturne fallback card + digest + reload/copy-details, reset-on-nav via `key={pathname}`) + real `NotFound` 404 route replacing the silent redirect. Frontend-only (all create endpoints pre-existed). Closes gaps.md 1.5.
- **N7 — Polish sweep:** motion audit, light-theme AA audit across all screens, empty states with CTAs, copy-feedback microstates, mockup-parity check. **N7 — DONE (2026-07-13):** legacy color aliases retired (`text-muted`→`text-ink-mute`, `text-faint`→`text-ink-faint`, `shadow-card`→`shadow-elev-1`) with a new `no-legacy-alias` guard test + dead var defs removed; `SecretTable` dead-sticky removed (its `overflow-x-auto`/`overflow-hidden` ancestor can't host a sticky header — now honestly non-sticky, per the N4 OpsTable decision); ⌘K palette gains **action** commands (New project → `/projects?new=1`, Export audit CSV, Toggle theme) alongside the existing nav — still names-only for secrets; `SecretEditor` copy-error danger toast + "Add secret" empty-state CTA (focuses the add-key input). Motion already shipped (N1) — verified applied (41 uses/20 files); light-theme AA audit clean (`ink-faint` confined to small/secondary metadata); six-screen mockup-parity clean. Frontend-only. **This is the FINAL Nocturne phase — N1–N7 complete the redesign.**
...
**Status (2026-07-13): COMPLETE.** All rollout phases N1–N7 are merged/ready (N7 in PR pending final holistic review). The Nocturne redesign is done: authority swapped to `docs/design/ui-nocturne-mockup.html` + N1 spec (N1), home command center + `/projects` (N2), secret editor restyle (N3), admin/ops surfaces restyle (N4), Settings hub + OIDC login (N5), ops create flows + ErrorBoundary/404 (N6), and the final polish sweep — alias retirement, palette actions, microstates, AA/motion/parity verification (N7). No security invariant from §1 moved across the program.
