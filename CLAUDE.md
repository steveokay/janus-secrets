...
- **Auth methods:** (1) user email + password (Argon2id) with session cookies for the UI, optionally gated by a TOTP second factor; (2) service tokens (long-lived, scoped, shown once at creation); (3) OIDC — generic OIDC provider config, tested against GitHub and Google; (4) OIDC-federated machine identity for CI (GitHub Actions JWT exchange); (5) WebAuthn passkeys for UI login — enrolled per user from Settings, and a single-step sign-in on their own (Janus requires user verification on every ceremony, so a passkey is possession + PIN/biometric and is not additionally gated by TOTP). The Relying Party ID and origins come from `JANUS_WEBAUTHN_RP_ID` / `JANUS_WEBAUTHN_ORIGINS`, never from the request Host, and are validated at boot; unset disables passkeys. Removing the last passkey never locks a user out — the password path always remains.
