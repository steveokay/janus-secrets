...
- OIDC/JOSE exception (approved 2026-07-07): OIDC login (sub-project C) uses audited third-party libraries — `github.com/coreos/go-oidc/v3`, `golang.org/x/oauth2`, and (transitively) `github.com/go-jose/go-jose/v4` — for JWT/JWKS verification, rather than hand-rolling JOSE. This is one of two recorded third-party crypto-adjacent exceptions (see WebAuthn below); the envelope/transit/unseal crypto remains stdlib + `x/crypto` only.
- WebAuthn exception (approved 2026-07-25): passkey login (roadmap 7.5) uses the audited third-party library `github.com/go-webauthn/webauthn` — and transitively `github.com/fxamacker/cbor/v2`, `github.com/go-webauthn/x`, and `github.com/google/go-tpm` — for COSE key parsing, attestation-statement verification, and assertion signature verification, rather than hand-rolling CBOR/COSE and the attestation formats. This is the second and final recorded third-party crypto-adjacent exception; the envelope/transit/unseal crypto remains stdlib + `x/crypto` only. Only PUBLIC credential material is stored (credential id + COSE public key + attestation metadata), so no master-key wrapping is involved and master-key rotation has nothing to re-wrap.

## AuthN / AuthZ

- **Auth methods:** (1) user email + password (Argon2id) with session cookies for the UI, optionally gated by a TOTP second factor; (2) service tokens (long-lived, scoped, shown once at creation); (3) OIDC — generic OIDC provider config, tested against GitHub and Google; (4) OIDC-federated machine identity for CI (GitHub Actions JWT exchange); (5) WebAuthn passkeys for UI login — enrolled per user from Settings, and a single-step sign-in on their own (Janus requires user verification on every ceremony, so a passkey is possession + PIN/biometric and is not additionally gated by TOTP). Two login ceremonies exist, in separate challenge pools: email-identified (account comes from the challenge) and **passwordless/discoverable** (no email; the account is resolved from the presented credential id in our own store, and the assertion's `userHandle` is only cross-checked against that owner — never used to select the account). Enrolment sets `residentKey: required` so new passkeys always work passwordlessly, and records the `credProps.rk` hint so the UI can report per-credential discoverability (`true`/`false`/unknown). The Relying Party ID and origins come from `JANUS_WEBAUTHN_RP_ID` / `JANUS_WEBAUTHN_ORIGINS`, never from the request Host, and are validated at boot; unset disables passkeys. Removing the last passkey never locks a user out — the password path always remains.
