...
| `/members` | Members — scoped RBAC bindings at instance / project / environment. **Role at \<scope\>** is the *effective* role the server will allow (the union of the user's own binding and any held through a group) and a **Source** column says which — `direct`, `via <group>`, or both — so a role nobody granted directly is never a mystery. The dropdown and Remove act on the **direct** binding only; a group-derived row links to `/groups`, where the grant actually lives. Also a **Groups** section binding a group at the selected scope (viewer / developer / admin; owner is not offered because the server refuses it) |
| `/access` | **Access review** — the cross-scope half of Members: a people × scopes grid (instance, each project, each of its environments) where a cell is the *effective* role there plus a note saying what produced it (`instance` / `project` / `env` / `group`). It exists because permissions union with no deny rules, so an environment with no binding of its own still has people who can write it. Clicking a person lists every binding they hold plus any live break-glass grant, and **Revoke all direct bindings** offboards in one action — removing direct bindings only, at scopes you may manage, never above your own *durable* role, never the last instance owner, and never group-derived access or a break-glass grant. The result names exactly what was left behind, and is marked complete only when nothing was. Every response reports its own limits: `instance_visible` when instance bindings are outside the caller's view, plus per-bound truncation flags |
...
Groups, `g v` Access review, `g s` Settings, … — the full table lives in the `?` modal). Chords never fire while
