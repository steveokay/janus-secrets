...
| `internal/authz` | Pure deny-by-default RBAC engine (viewer/developer/admin/owner; instance/project/env scopes; `Can`, `EffectiveRole`, grant/revoke). Bindings arrive from up to three sources that all resolve through one union rule: direct user bindings, group bindings (`WithGroups`, subject = a group the user belongs to), and the break-glass overlay (`WithGrants`, excluded from `BoundRole` so an elevation cannot become durable). `AllowedProjects` answers the same question in bulk with one binding load, for scoped audit read | ✅ implemented |
...
| `internal/nethard` | Shared SSRF-hardening for every operator-configured outbound caller: a connect-time `net.Dialer.Control` guard that re-checks the **resolved** IP on each dial (defeating DNS rebinding), an unconditional link-local/cloud-metadata block, an optional private-space block with a CIDR **allowlist** exempting only that block, redirect caps, and a proxy escape hatch that is off by default. Policy resolves through one process-wide `Source` read per dial, so a runtime change applies without a restart | ✅ implemented |
| `internal/notification` | Audit-tailing alert dispatcher + delivery outbox: webhook, Slack and SMTP channels for rotation/sync failures, denials and pending approvals | ✅ implemented |
| `internal/editreq` | Protected-config (four-eyes) edit requests: envelope-encrypted proposed changes, claim-before-commit approval, self-approval refused | ✅ implemented |
| `internal/auditship` | Ships the audit log as JSONL to a webhook (optional HMAC signature) or RFC 5424 syslog past a durable high-water mark — at-least-once, no gaps | ✅ implemented |
| `internal/backupsched` | Scheduled encrypted backups to S3-compatible storage: retention prune, `backup_runs` history, restore rehearsal that verifies without clobbering live data | ✅ implemented |
| `internal/metrics` | Hand-rolled zero-dependency Prometheus exposition, token-gated; HTTP metrics keyed by chi route pattern for bounded cardinality | ✅ implemented |
| `internal/meta` | Shared metadata helpers used across repositories | ✅ implemented |
| `internal/version` | Build version/commit/date, injected by ldflags and reported by `GET /v1/sys/version` | ✅ implemented |
| `internal/web` | `go:embed` target holding the built SPA assets served by the binary | ✅ implemented |
