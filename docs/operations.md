...
| `janus rotation create --config <id> --key <k> --type postgres\|webhook\|mysql\|redis\|oauth\|aws_iam --interval-seconds <n> [...]` | Create a rotation policy on a config's secret key. **Apply-a-value** types (Janus generates the value): `postgres`: `--admin-dsn`, `--role`, `--password-len` (default 32). `webhook`: `--url`, `--hmac-key`. `mysql`: `--admin-dsn` **or** `--mysql-addr`/`--mysql-admin-user`/`--mysql-admin-password`, plus `--mysql-user` (+ `--mysql-host`, `--mysql-tls`). `redis`: `--redis-addr`, `--redis-admin-user`/`--redis-admin-password`, `--redis-user` (+ `--redis-rules`, `--redis-tls`/`--redis-skip-verify`). **Generating** types (the external system mints the value): `oauth`: `--oauth-token-url`, `--oauth-client-id`, `--oauth-client-secret` (+ `--oauth-scope`/`--oauth-audience`) — stores the returned `access_token`; the previous token may stay valid until it expires. `aws_iam`: `--iam-user`, `--iam-region`, `--iam-access-key-id`, `--iam-secret-access-key` (+ `--iam-session-token`) — creates a new IAM access key, stores it as JSON, deletes the old key(s); AWS caps a user at 2 keys. Any type: optional `--notify-url`/`--notify-hmac-key`. Requires `rotation:manage`. See the rotation runbook (`docs/ops/rotation.md`) |
