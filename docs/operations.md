...
| `janus rotation create --config <id> --key <k> --type postgres\|webhook\|mysql\|redis --interval-seconds <n> [...]` | Create a rotation policy on a config's secret key. `postgres` type: `--admin-dsn`, `--role`, `--password-len` (default 32). `webhook` type: `--url`, `--hmac-key`. `mysql` type: `--admin-dsn` **or** `--mysql-addr`/`--mysql-admin-user`/`--mysql-admin-password`, plus `--mysql-user` (+ `--mysql-host`, `--mysql-tls`). `redis` type: `--redis-addr`, `--redis-admin-user`/`--redis-admin-password`, `--redis-user` (+ `--redis-rules`, `--redis-tls`/`--redis-skip-verify`). Any type: optional `--notify-url`/`--notify-hmac-key`. Requires `rotation:manage`. See the rotation runbook (`docs/ops/rotation.md`) |
