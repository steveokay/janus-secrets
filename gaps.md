...
| 1.15 | **No Integrations hub.** "Connect Janus to an external system" is scattered: GitHub Actions **sync** + Kubernetes **sync** live under `/operations`, while OIDC **login** provider + GitHub Actions **CI federation** live under `/settings`. There is no single catalog of external connectors, hurting discoverability. Proposal: a new `/integrations` **consolidating hub** (catalog of connector cards → GitHub Actions, Kubernetes, OIDC, CI Federation) that owns *connect & configure* and deep-links into `/operations` for *run & monitor* (keep that boundary — Sync is both). Phase 1 = frontend-only catalog/nav over existing endpoints (no backend); pull OIDC + federation config out of Settings into it later. Boundary (move Sync config wholesale vs. catalog-only) needs a brainstorm/spec before building. | nav in `web/src/shell/Sidebar.tsx`; sync under `web/src/operations/`, OIDC/fed under `web/src/settings/` | MED |
