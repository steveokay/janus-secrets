...
export function ConfirmDialog({ open, onOpenChange, title, body, confirmLabel, tone = 'brand', onConfirm }: {
  ...
  return (
    <AD.Root open={open} onOpenChange={onOpenChange}>
      <AD.Portal>
        <AD.Overlay className="fixed inset-0 z-50 bg-ink/30 backdrop-blur-[8px]" />
        <AD.Content className="fixed left-1/2 top-1/2 z-50 w-80 -translate-x-1/2 -translate-y-1/2 rounded-card border border-line bg-elevated p-5 shadow-pop">
