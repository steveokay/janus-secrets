...
func TestMain(m *testing.M) {
	...
	if err != nil {
		fmt.Println("projectkeys tests will be skipped: could not start postgres:", err)
		os.Exit(m.Run())
	}
	...
	if err == nil {
		if mErr := st.Migrate(ctx); mErr == nil {
			...
			fmt.Println("projectkeys tests will be skipped: migrate failed:", mErr)
			st.Close()
		}
	} else {
		fmt.Println("projectkeys tests will be skipped: open failed:", err)
	}
	code := m.Run()
	if testPool != nil {
		testPool.Close()
...
// the real read path and rotate/rewrap their KEK.
type harness struct {
	svc *Service
	sec *secrets.Service
	kr  *crypto.Keyring
...
// newHarness builds a rotation Service + secrets.Service over the shared store
// with a freshly unsealed keyring, skipping the test when Docker is absent.
func newHarness(t *testing.T) *harness {
	...
	svc := New(kr,
		store.NewProjectRepo(testStore),
		store.NewProjectKEKVersionRepo(testStore),
		store.NewSecretRepo(testStore),
	)
	return &harness{svc: svc, sec: secrets.NewService(testStore, kr), kr: kr}
}
