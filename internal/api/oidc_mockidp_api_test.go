...
// endpoint that mints a signed ID token for scripted claims. Duplicated from
// internal/auth/oidc_mockidp_test.go since that helper is package-private.
type mockIdP struct {
	srv        *httptest.Server
	key        *rsa.PrivateKey
	keyID      string
...
func newMockIdP(t *testing.T, clientID string) *mockIdP {
	...
	mux.HandleFunc("/jwks", func(w http.ResponseWriter, r *http.Request) {
		mockJSON(w, jose.JSONWebKeySet{Keys: []jose.JSONWebKey{{
